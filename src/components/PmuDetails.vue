<template>
   <!-- Details contents -->
   <div class="overflow-hidden rounded-xl divide-y divide-gray-300 w-full">
        <div class="p-3 md:p-5 xl:px-14 xl:py-12 flex flex-col space-y-5">
          <div class="flex items-center px-3 pt-3 md:pt-0">
            <h2 class="flex-1 text-3xl sm:text-5xl font-bold leading-none text-gray-800 mr-auto">Pre and Post Care
              Agreement</h2>
          </div>
          <div class="md:grid md:grid-cols-12 gap-5 flex flex-col-reverse ">
            <div class="md:col-span-7 lg:col-span-8 xl:col-span-9">
              <div class="relative p-5 flex flex-col w-full overflow-hidden rounded-lg border border-gray-300 shadow-3 bg-white">
                <iframe
            ...
            <div class="md:col-span-5 lg:col-span-4 xl:col-span-3">
              <div class="relative p-5 flex flex-col w-full space-y-2 rounded-lg border border-gray-300 shadow-3 bg-white">
                <div class="flex items-center">
                  <h4 class="text-xl font-bold text-gray-800 leading-none">Send to customer</h4>
                  <div class="ml-auto" v-if="isSendToCustomer">
                    <Menu as="div" class="relative inline-block text-left">
                      <MenuButton
                        class="inline-flex items-center rounded-md border-0 border-gray-700 text-lg font-medium text-gray-800 shadow-0 hover:bg-peach-500 focus:outline-none outline-none focus:ring-2 focus:ring-peach-500 focus:ring-offset-2 ease-in-out duration-300 hover:border-peach-500"
                        :class="[
                          active
                ...
                <div v-if="isSendToCustomer" >
                  <ul role="list" class="divide-y divide-gray-300">
                    <li class="flex" @click="isEditCustomer = true">
                      <div class="ml-0">
                        <p class="text-sm font-semibold text-gray-900">Carla Siphron</p>
                        <p class="text-sm text-gray-700">(469) 578-8248</p>
                        <p class="text-sm text-gray-700">carlasiphron@gmail.com</p>
                      </div>
                    </li>
                  </ul>
                </div>
                ...
                <div class="relative flex-1 cursor-pointer" v-if="!isSendToCustomer">
                  <div class="relative ">
                    ...
                    <MagnifyingGlassIcon class="h-4 w-4 text-gray-900" aria-hidden="true" />
                    </div>
                ...
                <div class="absolute w-full mt-1 p-5 pb-3 flex flex-col overflow-hidden md:rounded-lg border border-gray-300 shadow-3 bg-white"
                ...
                <ul role="list" class="divide-y divide-gray-300 ">
                  <li v-for="person in people" :key="person.email" @click="isSendToCustomer = true; isOpenCustomers = false" class="flex py-4 cursor-pointer hover:bg-gray-200 px-5 ease-in-out duration-300">
                    <div class="ml-0">
                      <p class="text-sm font-semibold text-gray-800">{{ person.name }}</p>
                      <p class="text-sm text-gray-700">{{ person.tel }}</p>
                      <p class="text-sm text-gray-700">{{ person.email }}</p>
                    </div>
                  </li>
                </ul>
              </div>
 ...
 <DialogPopup :open-dialog="isCreateCustomer" @closeModal="isCreateCustomer = false">
      <div class="space-y-6 relative max-w-full" :style="`width:390px`">
        <DialogTitle as="h3" class="flex-1 text-5xl font-bold leading-none text-gray-800">
          Create customer
        </DialogTitle>
