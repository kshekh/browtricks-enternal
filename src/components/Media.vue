<template>
  <div class="relative flex flex-col min-h-screen w-full">
    ...
    <Alert :open-alert="isCloseAlert" @closeAlert="isCloseAlert = false" :show-details-btn="true">
      <template #icon>
        <ShortIcon class="w-4 mr-2 text-white" />
      </template>
      <template #title>Hello</template>
      <template #description>Description</template>
      <template #details>
        <div class="flex items-center">
          <div class="h-6 w-6 flex justify-center items-center flex-shrink-0 overflow-hidden rounded"><img
              class="aspect-square object-cover"
              src="https://images.unsplash.com/photo-1517841905240-472988babdf9?ixlib=rb-1.2.1&amp;ixid=eyJhcHBfaWQiOjEyMDd9&amp;auto=format&amp;fit=facearea&amp;facepad=2&amp;w=256&amp;h=256&amp;q=80"
              alt=""></div>
          <div class="ml-4">
            <div class="font-medium text-white">Screenshot...102531.png</div>
          </div>
          <div class="ml-auto">
            <ShortIcon class="w-4 mr-2 text-white" />
          </div>
        </div>
        <div class="text-grey-700 flex gap-1 items-center mt-2">
          <span>File Format is not supported</span>
        </div>
      </template>
    </Alert>
    <!-- End Alert -->
